...
const validationForumComments_idServerUC = async ({ forumComments_id, forumThreads_id, userCommunities_id }) => {
  ...
  if (!validator.isLength(data, { min: minLength, max: maxLength })) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'XqKteMWhD', messageID: 'Pp_CFyt_3' }] });
  }
  ...
  if (data.match(/^[\w\-]+$/) === null) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'LvpynfO4n', messageID: 'JBkjlGQMh' }] });
  }
  ...
  if (count !== 1) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'a4iFJPINd', messageID: 'cvS0qSAlE' }] });
  }
