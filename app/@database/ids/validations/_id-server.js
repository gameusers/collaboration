...
const validationIDs_idArrServer = async ({ required = false, valueArr, loginUsers_id }) => {
  ...
  try {
    ...
    if (!Array.isArray(valueArr)) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'SZ_1gq_l-', messageID: 'qnWsuPcrJ' }] });
    }
    ...
    if (valueArr.length === 0) {
      ...
      if (required) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'g4qvICdhA', messageID: 'cFbXmuFVh' }] });
      }
    ...
    for (let dataObj of valueArr.values()) {
      ...
      if (!validator.isLength(_id, { min: minLength, max: maxLength })) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'DPijzWxpl', messageID: 'Pp_CFyt_3' }] });
      }
      ...
      if (_id.match(/^[\w\-]+$/) === null) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'snLJ8V3Dg', messageID: 'JBkjlGQMh' }] });
      }
