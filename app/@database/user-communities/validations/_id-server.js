 ...
 * @return {Object} バリデーション結果
 */
const validationUserCommunities_idAndAuthorityServer = async ({ value, loginUsers_id }) => {
  ...
  const communityType = lodashGet(docUserCommunitiesObj, ['communityType'], 'open');
  ...
  if (communityType === 'closed') {
    ...
    if (!followedArr.includes(loginUsers_id)) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'oMsVg7jXy', messageID: 'DSRlEoL29' }] });
    }
