 ...
 */
const validationTermsOfService = ({ throwError = false, agree, agreedVersion }) => {
  ...
  try {
    ...
    if (!validator.isBoolean(String(agree))) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'eQyXgxrXo', messageID: 'PH8jcw-VF' }] });
    }
    ...
    if (!agree) {
      ...
      if (agreedVersion) {
        ...
        const datetimeCurrent = moment(process.env.NEXT_PUBLIC_TERMS_OF_SERVICE_VERSION).utc();
        
        if (datetimeConfirmed.isBefore(datetimeCurrent)) {
          throw new CustomError({ level: 'warn', errorsArr: [{ code: 'PklwqOLIC', messageID: 'Uh3rnK7Dk' }] });
        }
      ...
      // ---------------------------------------------
        ...
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'kD87Pnp1p', messageID: 'Uh3rnK7Dk' }] });
