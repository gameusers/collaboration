...
const Component = (props) => {
  ...
  const [keyword, setKeyword] = useState(lodashGet(props, ['userCommunityObj', 'keyword'], ''));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (validationDelete({ value: keyword }).error) {
        throw new CustomError({ errorsArr: [{ code: '8vjn4yMiQ', messageID: 'uwHIKBy7c' }] });
      }
  ...
  return (
    ...
    >
      ...
      <p>ユーザーコミュニティを削除します。コミュニティを削除する場合は予め掲示板などで、参加メンバーにその旨を伝えておいてください。一度削除すると元には戻せませんので、よく考えてから削除を行ってください。</p>
