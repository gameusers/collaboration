...
const validationVideo = ({ throwError = false, videoChannel, videoID }) => {
  ...
  try {
    ...
    if (!validator.isIn(videoChannel, ['youtube'])) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'P9AI5L2nd', messageID: 'PH8jcw-VF' }] });
    }
    ...
    if (videoChannel === 'youtube') {
      ...
      if (!validator.isLength(videoID, { min: 7, max: 14 })) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'SLkXcjg5V', messageID: 'Pp_CFyt_3' }] });
      }
      ...
      if (videoID.match(/^[\w\-]+$/) === null) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'OCP1jOfmG', messageID: 'JBkjlGQMh' }] });
      }
