...
const validationGamesURLID = ({ throwError = false, value }) => {
  ...
  try {
    ...
    if (!validator.isLength(data, { min: minLength, max: maxLength })) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'gcRHxM0zw', messageID: 'Uh3rnK7Dk' }] });
    }
    ...
    if (data.match(/^[\w\-]+$/) === null) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 's01Xlh0xY', messageID: 'JBkjlGQMh' }] });
    }
