...
const findUserCommunitiesList = async ({
  ...
  try {
    ...
    if (keyword) {
      lodashSet(conditionObj, ['localesArr.name'], { $regex: pattern, $options: 'i' });
    }
    ...
    for (let value1Obj of docArr.values()) {
      ...
      const imagesAndVideosThumbnailObj = lodashGet(value1Obj, ['imagesAndVideosThumbnailObj'], {});
      ...
      if (gamesArr.length > 0) {
        ...
        const sortedGamesArr = [];
        const gameCommunities_idsArr = lodashGet(value1Obj, ['gameCommunities_idsArr'], []);
        const gamesArr = lodashGet(value1Obj, ['gamesArr'], []);
