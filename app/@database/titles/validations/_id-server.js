...
const validationTitles_idsArrServer = async ({ arr }) => {
  ...
  if (!Array.isArray(arr)) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'UoOzTieo-', messageID: '3mDvfqZHV' }] });
  }
  ...
  for (let value of arr.values()) {
    ...
    if (!validator.isLength(value, { min: minLength, max: maxLength })) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: '3didaI31_', messageID: 'Pp_CFyt_3' }] });
    }
    ...
    if (value.match(/^[\w\-]+$/) === null) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'qqXNOf2nO', messageID: 'JBkjlGQMh' }] });
    }
