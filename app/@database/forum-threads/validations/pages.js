...
const validationUsersPagesName = ({ throwError = false, required = false, value }) => {
  ...
  try {
    ...
    if (validator.isEmpty(data)) {
      ...
      if (required) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'WUWiWildY', messageID: 'cFbXmuFVh' }] });
      }
    ...
    if (!validator.isLength(data, { min: minLength, max: maxLength })) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'sWa22OEoR', messageID: 'Uh3rnK7Dk' }] });
    }
...
const validationUsersPagesLanguage = ({ throwError = false, value }) => {
  ...
  try {
    ...
    if (!validator.isIn(value, ['ja'])) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'a-YV7n9UH', messageID: 'PH8jcw-VF' }] });
    }
