...
export default async (req, res) => {
  ...
  try {
    ...
    lodashSet(requestParametersObj, ['editDeveloperPublisherID'], editDeveloperPublisherID);
    ...
    if (!administrator) {
      ...
      statusCode = 403;
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'EacibsVQF', messageID: 'DSRlEoL29' }] });
    ...
    if (!editDeveloperPublisherID) {
      ...
      if (count !== 0) {
        throw new CustomError({ level: 'warn', errorsArr: [{ code: 'nHu2qQAkf', messageID: '7B6RuzZGZ' }] });
      }
