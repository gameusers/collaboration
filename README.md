...
[docker.sh](https://github.com/gameusers/collaboration/blob/master/docker.sh) の実行は初回だけです。以後は以下のコマンドでサーバーを起動してください。これは Node.js のコンテナに入って npm run dev を実行するコマンドです。
