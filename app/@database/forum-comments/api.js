...
// const { validationForumComments_idServer } = require('./validations/_id-server');
...
router.post('/upsert-uc', upload.none(), async (req, res, next) => {
  ...
  try {
    ...
    const { userCommunities_id, forumThreads_id, forumComments_id, name, comment, imagesAndVideosObj } = req.body;
    ...
    //   await validationUserCommunities_idServer({ value: userCommunities_id });
    // } else {
    //   throw new CustomError({ level: 'warn', errorsArr: [{ code: '35ZONW7dQ', messageID: '1YJnibkmh' }] });
    // }
    ...
    // --------------------------------------------------
    //   DB find / Forum Comments / コメントが存在するかチェック
    // --------------------------------------------------
    ...
    if (!forumComments_id) {
      ...
      oldObj = lodashGet(forumCommentsObj, ['imagesAndVideosObj'], {});
      
      if (Object.keys(forumCommentsObj).length === 0) {
        throw new CustomError({ level: 'error', errorsArr: [{ code: '4FRXxeFxQ', messageID: 'cvS0qSAlE' }] });
      }
      
      
    // --------------------------------------------------
    //   DB find / Forum Comments / 同じ名前のスレッドが存在するかチェック
    // --------------------------------------------------
      
    } else {
      
      const dateTimeLimit = moment().utc().add(-365, 'day');
