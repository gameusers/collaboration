...
const validationGamesHardwareArrServer = async ({ required = false, localeObj, valueArr }) => {
  ...
  if (!Array.isArray(valueArr)) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'Gkrk8HeRT', messageID: 'qnWsuPcrJ' }] });
  }
  ...
  if (valueArr.length > limit) {
    throw new CustomError({ level: 'warn', errorsArr: [{ code: 'wdXYPnkw3', messageID: 'qnWsuPcrJ' }] });
  }
  ...
  if (valueArr.length === 0) {
    ...
    if (required) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'AI9tF19ZX', messageID: 'cFbXmuFVh' }] });
    }
  ...
  for (let valueObj of valueArr.values()) {
    ...
    if (!Number.isInteger(playersMin)) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'TwguaNB7k', messageID: 'f_YBnQcfW' }] });
    }

    if (!Number.isInteger(playersMax)) {
      throw new CustomError({ level: 'warn', errorsArr: [{ code: 'uIj9Oezb2', messageID: 'f_YBnQcfW' }] });
    }
